...
$requiredMainPatterns = @(
    ...
    '(?s)static uint8_t processAuxMoveCommand\(const char \*command\).*?motorId < AUX_MOTOR_MIN_ID.*?motorId > AUX_MOTOR_MAX_ID.*?distanceDmm == 0.*?mechanismLiftPulses.*?mechanismHorizontalPulses.*?Emm_V5_Pos_Control\(\(uint8_t\)motorId, direction, rpm,.*?accel, pulses, false, false\).*?motionMode = 3U.*?motionDuration = mechanismMotorDurationMs\(pulses, rpm\) \* 2U \+.*?AUX_MOVE_TIMEOUT_MARGIN_MS',
